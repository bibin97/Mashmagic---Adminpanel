...
const Dashboard = () => {
    ...
    const [taskFilter, setTaskFilter] = useState(7);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const [userRes, reportRes, examRes, distRes, taskRes] = await Promise.all([
                    api.get('/admin/users'),
                    api.get('/admin/mentor-head-report'),
                    api.get('/admin/exam-analytics'),
                    api.get('/admin/mentor-distribution'),
                    api.get(`/admin/task-analytics?days=${taskFilter}`)
                ]);
    ...
    useEffect(() => {
        const fetchTaskAnalytics = async () => {
            try {
                const res = await api.get(`/admin/task-analytics?days=${taskFilter}`);
                if (res.data.success) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-center mb-10">
                        ...
                        <select
                            value={taskFilter}
                            onChange={(e) => setTaskFilter(parseInt(e.target.value))}
                    ...
                    {/* Scrollable container for long date ranges */}
                    <div className="w-full h-[350px] overflow-x-auto overflow-y-hidden custom-scrollbar">
                        <div style={{ minWidth: taskPerformance.length > 15 ? `${taskPerformance.length * 40}px` : '100%', height: '100%' }}>
                            {isMounted && (
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <XAxis
                                            ...
                                            tickLine={false}
                                            dy={10}
                                            interval={taskPerformance.length > 30 ? 4 : (taskPerformance.length > 14 ? 1 : 0)}
                                        />
                                        <YAxis fontSize={10} fontWeight={600} tick={{ fill: '#cbd5e1' }} axisLine={false} tickLine={false} />
                                        <Tooltip
                                            contentStyle={{
                                                borderRadius: '12px',
                                                border: 'none',
                                        ...
                                        <Legend
                                            ...
                                            formatter={(value) => (
                                                <span className="text-slate-500 font-black text-[9px] uppercase tracking-widest ml-1">{value}</span>
                                            )}
                                        />
                                        <Bar
                                            name="Tasks"
